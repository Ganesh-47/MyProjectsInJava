...
public class ViewFeedback extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String url="";
		HashSet <Feedback> feedbackhashset=null;
		try {
			Controller admincontroller=new AdminController();
			feedbackhashset=admincontroller.viewAllFeedback();
			if(feedbackhashset!=null) {
				url="/ViewAllFeedback.jsp";
				request.setAttribute("feedbackhashset",feedbackhashset);
				getServletContext().getRequestDispatcher(url).include(request, response);
			}else {
				url="/Message.jsp";
				request.setAttribute("message", "Failed To View Feedback Details!!! ");
				getServletContext().getRequestDispatcher(url).include(request, response);
			}
