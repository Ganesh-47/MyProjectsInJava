...
public class ViewAllCategoryServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String url="";
		HashSet <Category> categoryhashset=null;
		try {
			Controller admincontroller=new AdminController();
			categoryhashset=admincontroller.viewAllCategory();
			if(categoryhashset!=null) {
				url="/ViewCategories.jsp";
				request.setAttribute("categoryhashset",categoryhashset);
				getServletContext().getRequestDispatcher(url).include(request, response);
			}else {
				url="/Message.jsp";
				request.setAttribute("message", "Failed To View Category Details!!! ");
				getServletContext().getRequestDispatcher(url).include(request, response);
			}
